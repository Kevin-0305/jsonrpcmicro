...
func NewServiceInfo(serviceId string, serviceName string, serviceAddress string) *ServiceInfo {
	return &ServiceInfo{
		ServiceId:      serviceId,
...
func NewClient(etcdSvc []string) (*Client, error) {
	// 配置 etcd
	config := clientv3.Config{
		Endpoints:   etcdSvc,          // etcd集群地址
...
func (this *Client) ParseService(key []byte, value []byte) {
	...
	if reg.Match(key) {
		...
		this.services = append(this.services, NewServiceInfo(string(serviceId), string(serviceName), string(value)))
	}
